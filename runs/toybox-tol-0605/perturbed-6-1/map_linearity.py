...
# def plot_error(epsilon, errors, ax, labels):
#     for i, lab in enumerate(labels):

#     ax.loglog(epsilon, errors, label=label)

if __name__ == "__main__":
    fig, ax = plt.subplots()
    # dot = False

    all_errors_1 = np.empty((4, 200))
    all_errors_2 = np.empty((4, 200))
    for iii, rtol in enumerate([1e-15, 1e-13, 1e-10, 1e-8]):
        ...
        for i, eps in enumerate(epsilon):
            try:
                errors_1[i] = linearized_error_jacobian(
                    pyoproblem.f_RZ_tangent, rtol=rtol, initpoint=manifold.inner["rfp_u"], vector=eps*manifold.inner["vector_u"],
                    eps=eps, dt = 2*np.pi
                )
            except:
                errors_1[i] = np.nan
        ...
        for i, eps in enumerate(epsilon):
            try:
                errors_2[i] = linearized_error(
                    pyoproblem.f_RZ, manifold.inner["rfp_u"], manifold.inner["vector_u"], eps, rtol=rtol, dt=2*np.pi
                )
            except:
                errors_2[i] = np.nan
        ...
        all_errors_1[iii] = errors_1
        all_errors_2[iii] = errors_2
    
    np.save("errors_jac.npy", all_errors_1)
    np.save("errors_dot.npy", all_errors_2)
    ...
    ax.set_xlabel("eps")
    ax.set_ylabel("error")
    fig.savefig("map_linearity.png", bbox_inches='tight', pad_inches=0.1, dpi=300)
