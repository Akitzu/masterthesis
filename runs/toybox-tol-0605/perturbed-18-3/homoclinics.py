...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--filename', type=str, default=None, help='Filename to save the plot')
    ...
    maxwellboltzmann = {"m": 18, "n": -3, "d": np.sqrt(2), "type": "maxwell-boltzmann", "amplitude": args.amplitude}
    ...
    while success is not True:
        if i == 0:
            try:
                ...
                eps_s_i, eps_u_i = eps_s_1, eps_u_1
            except Exception as e:
                print(e)
                print("Homoclinic point not found")
        else:
            guess_i = [eps_s_1*np.power(manifold.lambda_s, 1/2**i), eps_u_1*np.power(manifold.lambda_u, 1/2**i)]
            try:
                print(f"initial guess: {guess_i}")
                eps_s_i, eps_u_i = manifold.find_homoclinic(guess_i[0], guess_i[1], bounds = bounds_1)
            except Exception as e:
                print(e)
                print("Homoclinic point not found")
        ...
        fig.show()
        yn = input("Was the homoclinic point found? (y/n): ")
        if yn == "y":
            i += 1
        yn = input("Continue to find more homoclinic points? (y/n): ")
        if yn == "n":
            success = True
