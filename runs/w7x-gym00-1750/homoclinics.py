...
from horus.stellarator_configurations import stellarator
from horus.poincare import poincare
...
if __name__ == "__main__":
    ...
    bs, bsh, (nfp, coils, ma, sc_fieldline) = stellarator(w7x[0], currents, w7x[2], nfp=5, surface_radius=2)

    ### Creating the pyoculus problem object
    print("\nCreating the pyoculus problem object\n")
    pyoproblem = SimsoptBfieldProblem(ma.gamma()[0, 0], 0, 5, bs)
    ...
    fixedpoint.compute(guess=guess, pp=5, qq=4, sbegin=5.2, send=6.2)
    ...
    if args.compute_poincare:
        ...
        frac_nf_1 = 1/3
        nfieldlines_1, nfieldlines_2 = int(np.ceil(frac_nf_1*N_traj)), int(np.floor((1-frac_nf_1)*N_traj))+1
        ...
        # Computing poincare
        _, _, fig, ax = poincare(bsh, RZs, [0], sc_fieldline, tol = 1e-10)
    ...
    ax.set_title("W7-X GYM00 1750 Homoclinic Manifold")
